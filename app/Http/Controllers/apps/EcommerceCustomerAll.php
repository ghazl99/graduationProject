...
class EcommerceCustomerAll extends Controller
{
  public function UserManagement() {
    ...
    $verified = User::whereNotNull( 'email_verified_at' )->whereHas('roles', function ($query) {
      $query->where('name', 'العميل');
  })
    ->get()
    ->count();
    ...
    $roles = Role::where('name', 'العميل')->first();
  ...
  public function index( Request $request ) {
    ...
    if ( empty( $request->input( 'search.value' ) ) ) {
        $users = User::offset( $start )->whereHas('roles', function ($query) {
          $query->where('name', 'العميل');
      })
        ->limit( $limit )
        ->orderBy( $order, $dir )
